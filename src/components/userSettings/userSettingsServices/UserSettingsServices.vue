      ...
      <div id="body-div">
        <div v-for="service in services" :key="service.id" class="service-item">
          ...
          <div class="main">
            ...
            <div class="service-item-inner-wrapper">
              <div class="data-container">
                <div>
                  <p>{{ calculatePricing(service) }}</p>
                  <div id="equipment-list-window">
                    <p
                      v-for="item in service.selectedEquipment"
                      :key="item.index"
                      class="service-item-selected-equipment"
                    >
                      <b>{{ item.equipmentName }}</b
                      ><br />
                      Units Required: {{ item.unitsRequired }}
                    </p>
                    <p class="service-item-needed-employees">
        ...
        <div class="service-item">
          <router-link to="addnewservice" id="add-new-service-card">
...
//TODO: MAKE DEFAULT IMAGE FOR service
...
export default {
  ...
  methods: {
    calculatePricing(item) {
      if (item.flatRatePriceOption.rate !== 0) {
        return "$" + item.flatRatePriceOption.rate + " (flat rate)";
      } else if (item.hourlyPriceOption.baseRate !== 0) {
        return (
          "$" +
          item.hourlyPriceOption.baseRate.toLocaleString() +
          "/" +
          item.hourlyPriceOption.baseTime +
          "hr " +
          "+ " +
          "(" +
          item.hourlyPriceOption.additionalHourlyRate +
          "/hr" +
          ")"
        );
      }
    ...
    editItem(id) {
      console.log(id);
      this.$router.push({ name: "editservice", params: { serviceid: id } });
    },
    // deleteItem(service) {
      // if (confirm("Are you sure you want to delete?")) {
      //   if (!service.photo) {
      //     this.deleteItemFromFirestore(service.id);
      //     console.log(service.id);
      //     console.log("deleted without photo");
      //   } else if (service.photo) {
      //     this.deleteItemFromFirestore(service.id);
      //     this.deleteItemFromStorage(service.id);
    ...
    //     let servicePhotoRef = firebase
    //       .storage()
    //       .ref()
    //       .child("services/" + id + ".jpeg");
    //     servicePhotoRef
    //       .delete()
    //       .then(() => {
    ...
    //     .then(() => {
    //       console.log("deleted");
    //       const indexToDelete = this.services.findIndex((x) => x.id === id);
...
.service-heading-div h4 {
  width: 100%;
  text-align: center;
  padding: 10px;
}

.service-item-selected-equipment,
.service-item-needed-employees {
  font-size: 10px;
}
